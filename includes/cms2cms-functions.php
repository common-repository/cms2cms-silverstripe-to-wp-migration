...
/**
 * Class CmsPluginFunctionsSilverStripe
 */
class CmsPluginFunctionsSilverStripe
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback" : "//wordpress.org/support/plugin/cms2cms-silverstripe-to-wp-migration/reviews",
    ...
    {
        $cms2cms_silverstripe_access_login = get_option('cms2cms-silverstripe-login');
        $cms2cms_silverstripe_access_key   = get_option('cms2cms-silverstripe-key');

        return array(
            'email'     => $cms2cms_silverstripe_access_login,
            'accessKey' => $cms2cms_silverstripe_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_silverstripe_access_key != false);
    }  
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-silverstripe-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-silverstripe-key', $key);

            delete_option('cms2cms-silverstripe-login');
            add_option('cms2cms-silverstripe-login', $login);

            delete_option('cms2cms-silverstripe-depth');
            add_option('cms2cms-silverstripe-depth', $bridge_depth);
    ...
    {
        return $this->__('SilverStripe', 'cms2cms-silverstripe-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-silverstripe-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-silverstripe-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-silverstripe-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if (isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_silverstripe_logout')
                && $_POST['cms2cms_silverstripe_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
